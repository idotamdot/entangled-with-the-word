...
# Flattened master list for quick reference
PARABLES_OF_JESUS = [p for sub in PARABLES_BY_CATEGORY.values() for p in sub]


def render_parables_list():
    """Display a list of Jesus' parables."""
    st.header("📕 Parables of Jesus")
    for category, items in PARABLES_BY_CATEGORY.items():
        st.subheader(category)
        for parable in items:
